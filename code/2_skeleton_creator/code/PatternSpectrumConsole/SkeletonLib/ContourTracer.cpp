...
RasterPoint* ClosedPath::initialPoint() {
  return (path == nullptr) ? nullptr : path->first();
}
...
bool ContourTracer::getNextBorderPoint(int& X, int& Y) {
  ...
  while (yCurr < h) {
    // если стоим в начале байтика и он весь белый, можно сразу перейти на его
    // конец
    if (!((imageRaster->BITS_IN_BYTE - 1) & xCurr) &&
        (imageRaster->byteIsFalse(xCurr, yCurr) ||
         imageRaster->byteIsTrue(xCurr, yCurr))) {
...
int ContourTracer::extractPolygonOfMinimalPerimeter(ClosedPath* CP, int n) {
    ...
    do {
      {
        ...
        while (B) {
          ...
          {
            if (scal(Corner, left, test) < 0) { /* test левее левого створа */
              Corner = left;
              B = false;
          ...
          {
            if (scal(Corner, right, test) >
                0) { /* test правее правого створа */
              Corner = right;
              B = false;
  ...
  } else /* n<=3 */
  {
    ...
    test = test->getNext();
    while (test != nullptr) {
      if ((test->color == FirstPoint->color) &&
          ((test->x != FirstPoint->x) || (test->y != FirstPoint->y)))
        test->kind = 1;
      test = test->getNext();
    }
  }
...
ClosedPath* ContourTracer::initialContour() { return contourList->first(); }
...
void ContourTracer::traceContours() {
  ...
  while (setGrip(S)) {
    ...
    fillPolygonArea(CP);
    if (CP->Area < areaIgnore) delete CP;
  }
  delete S;
}
