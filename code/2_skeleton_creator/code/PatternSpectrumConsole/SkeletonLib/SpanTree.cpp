...
enum EventKind { EastCape, WestCape, PassSite, AddDisc, RemoveDisc };
/*типы событий*/
typedef Element TSite;
typedef Point TPoint;

class TEventPoint
    : public LinkedListElement<TEventPoint> /* событие-изменение статуса */ {
 public:
  TSite *Site;
  TSite *St1, *St2;
...
class TPriorLine {
 public:
  LinkedListTail<TEventPoint>
      *EvList;        /*список статических событий "концы отрезков"*/
...
TEventPoint::TEventPoint(TSite *S, TSite *SPred, TSite *Ssuc, bool Bubble) {
  ...
  if (PP == SP) { /*проходной узел*/
    ...
    Prod = (EdP->dest->X - EdP->org->X) * (EdS->dest->Y - EdS->org->Y) -
           (EdP->dest->Y - EdP->org->Y) * (EdS->dest->X - EdS->org->X);
    if (PP && (!SP)) { /*западный мыс*/
      if (Bubble)
        Kind = AddDisc;
      else
        Kind = WestCape;
...
TEventPoint::TEventPoint(TSite *S, TDisc *D)
    : Site(S), Pt(new TPoint(D->X - D->Rad, D->Y)), Kind(RemoveDisc) {}

TEventPoint::~TEventPoint() {
  if (Kind == RemoveDisc) delete Pt;
...
int TEventPoint::ComparePos(TEventPoint *Ev) {
  ...
  else if (Pt->X == Ev->Pt->X) {
    if (Pt->Y < Ev->Pt->Y)
      result = -1; /*абсциссы совпадают*/
    else if (Pt->Y == Ev->Pt->Y)
      result = 0;
  }
  return result;
}
...
TPriorLine::TPriorLine(Domain *Collect) {
  EvList = new LinkedListTail<TEventPoint>(), EvTree = new TAVL(CompareEvents),
  XPos = 0.0;
  vector<TEventPoint *> MyList;
  int i = 0;
  TSite *E, *E1, *E2;
  TEventPoint *EvP = nullptr;
  // inherited::Create();
...
void TopologySort(Domain *Collect) {
  ...
  while (S != nullptr) {
    S1 = S->getNext();
    if (S->ClosestSite != nullptr) {
      St = S->ClosestSite->Cont;
      S->Container = St;
      if (St->MySons == nullptr) St->MySons = new LinkedListTail<TContour>;
...
void SpaningTree(Domain *Collect) {
  ...
  while (!PriorLine->empty()) {
    ...
    // debug
    // d_Output().clear(); d_Output().s() << "EvP " << PriorLine->XPos << " " <<
    // EvP->Pt->X << " " << EvP->Pt->Y << endl; d_Output().show();
...
TEventPoint *TPriorLine::MinEvent() {
  TEventPoint *result = nullptr;
  TEventPoint *EvT = nullptr, *EvL = nullptr;
...
TPriorLine::~TPriorLine() {
  while (!EvList->isEmpty()) delete EvList->first();
  delete EvList;
  delete EvTree;
}
...
void TStatus::WorkEvent(TEventPoint *EvP) {
  TSiteRecord *SR = nullptr, *SRA = nullptr, *SRB = nullptr, *SAb = nullptr, *SBe = nullptr,
              *SRab = nullptr;
  TDiscRecord *SD = nullptr;
  switch (EvP->Kind) {
    case EastCape: { /*добавление восточного мыса в сатус*/
    ...
    case WestCape: { /*удаление западного мыса из статуса*/
      SR = Find(EvP->St1);
      SRB = Above(SR);
      Remove(SRB);
    ...
    case PassSite: { /*проходной промежуточный сайт*/
      SR = Find(EvP->St1);
    ...
    case AddDisc: { /* западный мыс - с пузырем */
      SR = Find(EvP->St1);
      SRB = Above(SR);
      if (SRB != nullptr) {
        Remove(SRB);
      ...
      SRA = Above(SD);
      SRB = Below(SD);
      if (SRA != nullptr) SD->Correction(SRA->Site);
      if (SRB != nullptr) SD->Correction(SRB->Site);
    } break;
    case RemoveDisc: {
      SR = Find(EvP->Site);
      SD = (TDiscRecord *)SR;
      SD->Site->Cont->ClosestSite = SD->BornSite;
      if (SD->BornSite != nullptr) SD->Site->Cont->Container = SD->BornSite->Cont;
      SRA = Above(SR);
      SRB = Below(SR);
      Remove(SR);
      delete SD;
...
TDiscRecord::TDiscRecord(TSite *St)
    : TSiteRecord(St),
      Disc(new TDisc(-1000 - ((Vertex *)(St))->p->Y, 0, ((Vertex *)(St))->p->X,
                     true)),
      BornSite(nullptr),
...
void TDiscRecord::Correction(TSite *St) {
  TPoint *r, *r0, *r1;
  double ax = 0.0, ay = 0.0, bx = 0.0, by = 0.0, S = 0.0, M = 0.0, t = 0.0,
         lam = 0.0; /*коррекция пузыря */
  t = -1;
  if (St->IsVertex()) {
    r0 = ((Vertex *)St)->p;
    r = ((Vertex *)Site)->p;
...
/*TRUE если S1 ниже S2 в точке x*/
{
  ...
  if (S1->IsVertex()) {
    V1 = (Vertex *)S1;
    if (S2->IsVertex()) { /*оба сайта - точки*/
      V2 = (Vertex *)S2;
      result = (V1->p->Y < V2->p->Y) ||
               ((V1->p->Y == V2->p->Y) && (S1->NEl < S2->NEl));
...
TSiteRecord::TSiteRecord(TSite *St) { Site = St; }
...
void TStatus::Remove(TSiteRecord *SRec) {
  if (SRec != nullptr) StTree->Remove(SRec);
  //    View;
}

void TStatus::Replace2(TSiteRecord *SRec, TSiteRecord *SBelow,
                       TSiteRecord *SAbove)
/*замена записи двумя другими*/
{
  StTree->Remove(SRec);
...
void TStatus::Neighbours(TSiteRecord *SRec, TSiteRecord *SAdj) {
  ...
  if (((SRec == nullptr) || (SAdj == nullptr)) ||
      (dynamic_cast<TDiscRecord *>((SRec)) &&
       dynamic_cast<TDiscRecord *>((SAdj))))
    return;
  else if (dynamic_cast<TDiscRecord *>(SRec)) {
    SD = (TDiscRecord *)SRec;
    SD->Correction((SAdj->Site));
  } else if (dynamic_cast<TDiscRecord *>(SAdj)) {
    SD = (TDiscRecord *)SAdj;
...
/*Коррекция смежных кругов, лежащих выше*/
{
  TDiscRecord *SD;
  TSiteRecord *SAdj;
  if (SRec == nullptr) return;
  SAdj = Above(SRec);
  while ((SAdj != nullptr) && dynamic_cast<TDiscRecord *>((SAdj))) {
    SD = (TDiscRecord *)SAdj;
    SD->Correction((SRec->Site));
...
int CompareRecords(void *p1, void *p2) {
  ...
  if (RecP1->Site == RecP2->Site)
    result = 0;
  else if (Lower(RecP1->Site, RecP2->Site))
    result = -1;
  else
    result = 1;
  return result;
...
TStatus::TStatus(TPriorLine *) { StTree = new TAVL(CompareRecords); }
...
void TStatus::View() /*визуализация*/
{
  ...
  int i;
  Kn = StTree->Ruler->first();
  for (i = 0; i != NMem; i++) Memb[i] = 0;
  i = 0;
  while ((Kn != nullptr) && (i <= NMem)) {
    SR = (TSiteRecord *)Kn->key;
    Memb[i] = SR->Site->NEl;
    Kn = Kn->getNext();
    i++;
