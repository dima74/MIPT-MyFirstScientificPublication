...
class LinkedListElement {
 ...
 public:
  ...
  virtual ~LinkedListElement() { removeFromCurrentList(); }
  T* getNext() {
    if (next && next->isTail) {
      return NULL;
    }
...
// el1 -> el2 -> TAIL -> el1
template <class T>
class LinkedListTail : public LinkedListElement<T> {
 ...
 public:
  ...
  T* first() { return (T*)_first(); }
  T* last() { return (T*)_last(); }
  int cardinal() {
    int count = 0;
