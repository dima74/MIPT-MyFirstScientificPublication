...
class Element
    : public LinkedListElement<
          Element> /* Элемент коллекции (сайт) - вершина или ребро */ {
  friend class Couple;
  friend class Domain;
  friend class Edge;
  friend class Point;
...
class Couple
    : public LinkedListElement<Couple> /* Пара - дуга в триангуляции */ {
  friend class Domain;
  friend class Edge;
  friend class Element;
  friend class Point;
...
class Triplet
    : public LinkedListElement<Triplet> /* Тройка - симплекс в триангуляции */ {
  friend class Couple;
  friend class Domain;
  friend class Edge;
  friend class Element;
 ...
 public:
  ...
  Triplet *t1, *t2,
      *t3;     /* Смежные симплексы против ЧС: t1=>e1=>t2=>e2=>t3=>e3 */
...
class TriangleMap
    : public LinkedListElement<
          TriangleMap> /* Карта смежности коллекции точек и сегментов */ {
  friend class Couple;
  friend class Domain;
  friend class Edge;
  friend class Element;
...
class TContour
    : public LinkedListElement<
          TContour> /* Замкнутый контур - граница области. Область слева. */ {
  friend class Couple;
  friend class Domain;
  friend class Edge;
  friend class Element;
 ...
 public:
  int Number;                            /*Порядковый номер*/
  LinkedListTail<Point>* ListPoints;     /* Список точек  !!!! NEED points here! */
...
class Domain {
 ...
 public:
  LinkedListTail<TContour>*
      Boundary; /* Список замкнутых контуров - границ объекта */
