...
TKnot::TKnot() : key(0), left(0), right(0) { bal = 0; }
...
/* Установка элемента в АВЛ-дерево*/
{
  ...
  if (p == 0) /* Включить новый узел */
  {
    ...
    p->key = X;
    //       p.count:=1;
    if (Ruler->isEmpty()) p->moveIntoTail(Ruler);
  } else if (Compare(p->key, X) == 1) /*x встраивается в левое поддерево*/
  {
    NewKnot = (p->left == 0);
    Search(X, p->left, h);
    if (NewKnot && (p->left != 0)) p->left->moveAsPrevFor(p);
  ...
  } else if (Compare(p->key, X) == -1) /*x встраивается в правое поддерево*/
  {
    NewKnot = (p->right == 0);
    Search(X, p->right, h);
    if (NewKnot && (p->right != 0)) p->right->moveAsNextFor(p);
    ...
    ; /*p.Key=x */ /*x совпал с ключем узла p*/
                   //       p.count:=p.count+1;
}
...
/*Поиск узла с ключем x в поддереве с корнем p*/
{
  ...
  if (p == 0) /* такого ключа нет */
    ...
    if (Comp == 1) /*x находится в левом поддереве*/
