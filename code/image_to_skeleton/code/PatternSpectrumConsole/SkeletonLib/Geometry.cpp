...
/*Положение точки q относительно вектора p1,p2 */
{
  ...
  double ax = 0.0, ay = 0.0, bx = 0.0, by = 0.0, S = 0.0, C = 0.0, ma = 0.0,
         mb = 0.0;
  ax = P2->X - P1->X;
  ay = P2->Y - P1->Y;
  bx = q->X - P1->X;
...
/*монотонность трех собственных точек*/
{
  bool result = false;
  result =
      ((r2->X - r1->X) * (r1->X - r0->X) + (r2->Y - r1->Y) * (r1->Y - r0->Y) >
       0);
  return result;
}
...
/*коллинеарность трех собственных точек*/
{
  bool result = false;
  result =
      ((r1->X - r0->X) * (r2->Y - r0->Y) == (r1->Y - r0->Y) * (r2->X - r0->X));
  return result;
}

void LinSyst(double A1, double b1, double C1, double A2, double b2, double C2,
...
void Circle(double r0x, double r0y, double r1x, double r1y, double r2x,
            double r2y, double *X, double *Y, double *Rad) {
...
bool RightOrientation(Point *r0, Point *r1, Point *r2, Point *r3, Point *r4,
                      Point *r5)
...
TDisc *P3(Point *r0, Point *r1, Point *r2) {
  ...
  double X = 0.0, Y = 0.0, u = 0.0, V = 0.0,
         r = 0.0; /* Построение круга, касающегося трех точек против ЧС! */
  X = (r1->X - r0->X);
  Y = (r1->Y - r0->Y);
  u = (r2->X - r0->X);
  V = (r2->Y - r0->Y);
...
{
  TDisc *result;
  double s1x = 0.0, s1y = 0.0, s2x = 0.0, s2y = 0.0, s3x = 0.0, s3y = 0.0,
         A1 = 0.0, A2 = 0.0, b1 = 0.0, b2 = 0.0, C1 = 0.0, C2 = 0.0, P1 = 0.0,
         P2 = 0.0, P3 = 0.0, D = 0.0, X = 0.0, Y = 0.0;
...
{
  TDisc *result;
  double e1x = 0.0, e1y = 0.0, e2x = 0.0, e2y = 0.0, ex = 0.0, ey = 0.0,
         sx = 0.0, sy = 0.0, t1 = 0.0, t2 = 0.0, l = 0.0, r1x = 0.0, r1y = 0.0,
         r2x = 0.0, r2y = 0.0, z1x = 0.0, z1y = 0.0, z2x = 0.0, z2y = 0.0,
         XC = 0.0, YC = 0.0, Rad = 0.0;
  e1x = (r1->X - r0->X);
  e1y = (r1->Y - r0->Y);
  e2x = (r2->X - r3->X);
  e2y = (r2->Y - r3->Y);
  ...
  sx = ey / t1;
  sy = -ex / t1;
  t2 = double(((r2->X - r4->X) * e2y - (r2->Y - r4->Y) * e2x)) /
       (sx * e2y - sy * e2x);
  if (abs(t2) < Mnull) {
    t2 = 0.0;
    if (!(r4 == r3) && !(r4 == r2)) {
  ...
  t1 = double(((r1->X - r4->X) * e1y - (r1->Y - r4->Y) * e1x)) /
       (sx * e1y - sy * e1x);
  if (abs(t1) < Mnull) {
    t1 = 0.0;
    if (!(r4 == r1) && !(r4 == r0)) {
...
TDisc *S1_P2(Point *r0, Point *r1, Point *r2, Point *r3) {
  TDisc *result;
  double ux = 0.0, uy = 0.0, vx = 0.0, vy = 0.0, wx = 0.0, wy = 0.0, al = 0.0,
         uv = 0.0, uw = 0.0, vw = 0.0, RRx = 0.0, RRy = 0.0, r1x = 0.0,
         r1y = 0.0, D = 0.0, t = 0.0, zx = 0.0, zy = 0.0, rx = 0.0, ry = 0.0,
         XC = 0.0, YC = 0.0, Rad = 0.0;
  ux = (r1->X - r0->X);
  uy = (r1->Y - r0->Y);
  vx = (r2->X - r0->X);
  ...
  if (abs(uw) > Mnull) { /* r2,r3 не перпендикулярен r0,r1 */
    vw = (vx * wy - vy * wx);
    al = vw / uw;
    rx = (r0->X) + ux * al;
    ry = (r0->Y) + uy * al;
  ...
  } else { /* r2,r3 перпендикулярен r0,r1 */
...
bool InterEdge(Point *r1, Point *r2, TDisc *C) {
  bool result = false;
  double u1 = 0.0, V1 = 0.0, u2 = 0.0, V2 = 0.0, u3 = 0.0, v3 = 0.0, r = 0.0,
         S = 0.0;
  u1 = (r1->X) - C->X;
  V1 = (r1->Y) - C->Y;
  u2 = (r2->X) - C->X;
...
bool SameCircle(TDisc *CircP, TDisc *CircQ) {
  ...
  if (E > 0.1) E = 0.1;
  result = ((abs(CircP->X - CircQ->X) < E) && (abs(CircP->Y - CircQ->Y) < E) &&
            (abs(CircP->Rad - CircQ->Rad) < E));
  return result;
} /*SameCircle*/
...
bool InterTriangle(Point *r0, Point *r1, Point *r2, Point *r) {
  bool result = false;
  double S1 = 0.0, S2 = 0.0,
         S3 = 0.0; /* Попадание точки r внутрь треугольника r0,r1,r2 */
...
bool InterPointEdge(Point *r, Point *r0, Point *r1, Point *r2, TDisc *Circ1,
                    TDisc *Circ2) {
  bool result = false;
  Point *P1, *P2;
  double
      X1 = 0.0,
      Y1 = 0.0, X2 = 0.0, Y2 = 0.0, m1 = 0.0,
      a = 0.0; /* Попадание точки r внутрь треугольника с вершинами: r0 и точки
...
bool InterTwoEdges(Point *r, Point *r1, Point *r2, Point *r3, Point *r4,
                   TDisc *Circ1, TDisc *Circ2) {
  bool result = false;
  double X1 = 0.0, X2 = 0.0, Y1 = 0.0, Y2 = 0.0, a = 0.0, B = 0.0, C = 0.0,
         m1 = 0.0;
